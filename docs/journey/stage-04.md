# Stage 04 · Recoverable log segments
...
??? note "File diff: tests/log/test_recovery.py"
    ```diff
...
??? note "File diff: tests/log/test_segment.py"
    ```diff
...
??? note "File diff: src/minikafka/log/recovery.py"
    ```diff
