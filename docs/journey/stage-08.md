# Stage 08 · Consumer position and replay
...
??? note "File diff: tests/consumer/test_delivery_semantics.py"
    ```diff
...
??? note "File diff: tests/consumer/test_positions.py"
    ```diff
...
??? note "File diff: tests/reliability/test_offset_restart.py"
    ```diff
