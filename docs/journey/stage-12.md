# Stage 12 · ISR and high watermark
...
??? note "File diff: tests/replication/test_follower_fetch.py"
    ```diff
...
??? note "File diff: tests/replication/test_high_watermark.py"
    ```diff
...
??? note "File diff: tests/replication/test_isr.py"
    ```diff
...
??? note "File diff: src/minikafka/replication/model.py"
    ```diff
...
??? note "File diff: src/minikafka/replication/replica.py"
    ```diff
