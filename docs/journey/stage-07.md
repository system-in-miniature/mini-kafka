# Stage 07 · Partitioned producer batching
...
??? note "File diff: tests/producer/test_batching.py"
    ```diff
...
??? note "File diff: tests/producer/test_ordering.py"
    ```diff
...
??? note "File diff: tests/unit/test_partitioner.py"
    ```diff
...
??? note "File diff: src/minikafka/producer/accumulator.py"
    ```diff
...
??? note "File diff: src/minikafka/producer/partitioner.py"
    ```diff
