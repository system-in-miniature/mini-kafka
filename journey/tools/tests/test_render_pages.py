#!/usr/bin/env python3
"""Contracts for MiniKafka browser-native Journey lessons."""
...
class RenderPagesTest(unittest.TestCase):
    ...
    def test_all_twenty_cards_load_with_lossless_patch_coverage(self) -> None:
        self.assertEqual([card.number for card in self.cards], list(range(1, 21)))
    ...
    def test_supporting_scaffold_is_one_collapsed_group(self) -> None:
        ...
        for chinese, drawer in (
            (False, '??? note "Supporting file diffs (3 files)"'),
            (True, '??? note "支撑文件差异（3 个文件）"'),
        ):
            with self.subTest(chinese=chinese):
    ...
    def test_main_generates_bilingual_indexes_and_twenty_pages(self) -> None:
        ...
        for root in (Path("docs/journey"), Path("docs/zh/journey")):
            ...
            self.assertEqual(len(list(root.glob("stage-*.md"))), 20)
    ...
    def test_site_exposes_three_modes_and_same_stage_language_switch(self) -> None:
        ...
        self.assertEqual(nav.count("journey/stage-"), 40)
