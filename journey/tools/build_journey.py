#!/usr/bin/env python3
"""Build, verify, and prepare MiniKafka Journey workspaces."""
...
def _tree_files(root: Path, paths: tuple[str, ...]) -> dict[str, bytes]:
...
def assert_tree_parity(
    ...
    *,
    ...
    rebuilt_files = _tree_files(rebuilt, roots)
    reference_files = _tree_files(reference, roots)
...
def check_chain(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="minikafka-journey-chain-") as raw:
        workspace = Path(raw)
...
def assert_stage_parity(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(
        prefix=f"minikafka-stage-{stage_number:02d}-"
    ) as raw:
        expected = Path(raw)
...
def main() -> int:
    ...
    if len(stages) != len(manifest.stages):
        raise JourneyError(
            f"manifest has {len(manifest.stages)} stages but artifacts have {len(stages)}"
        )
    ...
    default = ROOT.parent / "MiniKafka-journey-workspace"
