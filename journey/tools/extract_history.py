#!/usr/bin/env python3
"""Resolve MiniKafka's history-driven Journey snapshots from Git evidence."""
...
def load_manifest(path: Path) -> HistoryManifest:
    ...
    if not isinstance(project, dict):
        raise TypeError("manifest requires [project]")
...
def main() -> int:
    ...
    print(f"wrote {len(manifest.stages)} MiniKafka Stage patch/test pairs")
    return 0
