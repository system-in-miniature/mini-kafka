...
REPO_URL = "https://github.com/system-in-miniature/mini-kafka"
META = re.compile(
...
def role_rank(path: str) -> int:
    if path.startswith(("src/minikafka/core/", "src/minikafka/log/")):
        return 0
    if path.startswith("src/minikafka/"):
        return 1
...
def load_block_layouts(
    ...
    *,
    ...
    if not isinstance(raw_failure_files, list) or any(
        not isinstance(item, str) or not item for item in raw_failure_files
    ):
        raise ValueError(f"{stage_label}: failure_files must be a list of paths")
    failure_files = tuple(raw_failure_files)
    expected_failure_files = tuple(
        sorted(path for path in patch_paths if path.startswith("tests/"))
    )
    ...
    if not isinstance(raw_blocks, list):
        raise TypeError(f"{stage_label}: blocks must be a list")

    blocks: list[BlockLayout] = []
    for index, raw in enumerate(raw_blocks, start=1):
        if not isinstance(raw, dict):
            raise TypeError(f"{stage_label}: block {index} must be a table")
        ...
        if any(
            ...
            raise ValueError(
                f"{stage_label}: block {block_id} has an empty localized field"
            )
        files = raw["files"]
        if (
            ...
            raise ValueError(
                f"{stage_label}: block {block_id} requires a non-empty file list"
            )
        ...
        if not isinstance(supporting, bool):
            raise TypeError(
                f"{stage_label}: block {block_id} supporting must be boolean"
            )
        blocks.append(
            BlockLayout(
                id=block_id,
...
def _heading_positions(body: str, headings: tuple[str, ...], *, label: str) -> None:
    ...
    for heading in headings:
        ...
        if len(occurrences) != 1:
            raise ValueError(
                f"{label}: expected one {heading!r}, found {len(occurrences)}"
            )
...
def _parse_region_file_lessons(
    region: str,
    *,
    region_name: str,
    label: str,
    ...
    """Bind one explanation to one marker or a consecutive marker group."""
    ...
    while index < len(markers):
        ...
        while (
            last + 1 < len(markers)
            and not region[markers[last].end() : markers[last + 1].start()].strip()
        ):
            last += 1
        end = markers[last + 1].start() if last + 1 < len(markers) else len(region)
        file_body = region[markers[last].end() : end].strip()
        paths = [markers[item].group(1).strip() for item in range(index, last + 1)]
        if not file_body.startswith("#### "):
            raise ValueError(
                f"{label} {paths}: marker group requires one #### explanation"
            )
        for path in paths:
            ...
            if (path.startswith("tests/")) != (region_name == "test contract"):
                raise ValueError(
                    f"{label} {path}: tests belong in the test contract and production "
                    "files belong in mechanism blocks"
                )

        slices = tuple(
            _normalize_slice(match) for match in CODE_FENCE.findall(file_body)
        )
        for code in slices:
            if len([line for line in code.splitlines() if line.strip()]) > 15:
                raise ValueError(
                    f"{label} {paths}: key code must contain at most 15 nonblank lines"
                )
            if code and not any(
                _slice_in_patch(code, _post_patch_text(patch_by_path[path]))
                for path in paths
            ):
                raise ValueError(
                    f"{label} {paths}: key code does not match stage.patch"
                )
        files.extend(
            FileLesson(path=path, body=file_body, code_slices=slices) for path in paths
        )
        index = last + 1
    return files
...
def parse_localized_lesson(
    ...
    *,
    ...
    nested_failure_heading = (
        "#### 先看会坏在哪里" if chinese else "#### See the failure first"
    )
    ...
    if expected_test_paths:
        ...
        failure_matches = list(
            re.finditer(
                rf"^{re.escape(nested_failure_heading)}$", test_region, re.MULTILINE
            )
        )
        ...
        if first_marker is None or failure_matches[0].start() > first_marker.start():
            raise ValueError(
                f"{label}: failure preview must precede test file walkthroughs"
            )
    ...
    if actual_paths != expected_paths:
        ...
        raise ValueError(
            f"{label}: walkthrough coverage mismatch; missing={missing}, extra={extra}"
        )
...
def _render_file_explanation(
    ...
    *,
    ...
    if not separator:
        raise ValueError(
            f"{lesson.path}: file lesson has no explanation after its heading"
        )
    explanation = re.sub(
        r"^##### (.+)$",
...
def _render_mechanism_block(
    ...
    *,
    ...
    if block.supporting:
        ...
        return "\n\n".join(parts)
    lessons = [lesson_by_path[path] for path in block.files]
    shared_explanation = bool(lessons) and all(
        lesson.body == lessons[0].body for lesson in lessons
    )
    if shared_explanation:
        for path in block.files:
            parts.append(_render_file_diff(patch_by_path[path], chinese=chinese))
        parts.append(_render_file_explanation(lessons[0]))
        return "\n\n".join(parts)
    for path in block.files:
        if path.startswith("tests/"):
            continue
...
def render_card(card: Card, *, chinese: bool) -> str:
    ...
    patch_link = (
        f"{REPO_URL}/blob/main/journey/stages/{card.number:02d}-{card.slug}/stage.patch"
    )
    return (
    ...
    for card in cards:
        ...
        chapter_link = (
            ...
            + {
                ...
                2: "02-the-log.md",
                3: "03-retention-compaction.md",
                4: "04-producer.md",
                5: "05-replication-basics.md",
                6: "06-isr-and-fencing.md",
                7: "07-consumer-groups.md",
                8: "08-transactions.md",
                9: "09-delivery-semantics.md",
                10: "10-protocol-and-beyond.md",
            }[card.chapter]
        )
        lines.append(
